...
export const LAYERS: Record<LayerKey, LayerInfo> = {
  ...
  controller: {
    label: 'Entry point',
    name: 'Controller',
    owns: 'Controller logic is now separate from your business logic. Your LWC\'s, batch job\'s, queueable\'s, and REST endpoint\'s now all delegate business logic to the service layer.',
    ruleHeading: 'The rule',
  ...
  domain: {
    label: 'Layer',
    name: 'Domain',
    owns: 'Represents the behaviour of a single SObject, it often houses validations, default values, and any other object specific automations',
    ruleHeading: 'The rule',
    rule: 'A domain class should only represent the behaviour of a single object in your org, not multiple objects. Behaviour representative of multiple objects should live in the service layer',
    chapters: [9, 10, 11],
  },
  service: {
    label: 'Layer',
  ...
  uow: {
    label: 'Layer',
    name: 'Unit of Work',
    owns: 'Every DML transaction is collected and registered to be executed, then they are committed together in dependency order.',
    ruleHeading: 'The rule',
    rule: 'This helps ensure that no partially complete transactions occur (An Account is made, but the 15 contacts for it fail to be inserted).  All DML gets commited together and should something fail, savepoints can easily roll the whole thing back.',
    chapters: [5, 6],
  },
};

...
<YouTube id="TzRohBbp8dw" title="Unit Test Mocks with Separation of Concerns" duration="26:40" />

---

## How does Unit Testing fit into Separation of Concerns?

The answer to this is simple: without Separation of Concerns, there is no unit testing. It just simply isn't possible. To unit test you need to be able to [create stub (mock) classes](https://developer.salesforce.com/docs/atlas.en-us.apexcode.meta/apexcode/apex_testing_stub_api.htm) to send into your class you are testing via [dependency injection](https://en.wikipedia.org/wiki/Dependency_injection) (or through the use of a factory, more on this in the next section). If all of your concerns are in one class (DML transactions, SOQL queries, service methods, domain methods, etc) you cannot fake anything. (Worth remembering as you read on: mocking a class's *dependencies* isn't the only DML-free testing trick in the library - the domain layer also has its own built-in mock database for exercising trigger logic, covered in [chapter 11](/domain-layer/implementing/).) Let's take a look at a couple examples to illustrate this problem:
...
It's not a simple path, unfortunately. There is a lot of work ahead of you to start this transition, but it is possible. The key is to start small. If you are a tech lead, the first thing you need to do is find the time to train your devs on what SoC and mocking are and why you would use them. It's critical they understand the concepts before trying to roll something like this out. You cannot do everything as a lead even if you'd like to, you need to build your team's skillset first. If you aren't a lead, you first need to convince your lead why it's critical you start taking steps in that direction and work to get them on board with it. If they ignore you, you need a new lead... After accomplishing either of the above you should do the following:
...
## The Technical Order to Adopt the Layers In

That's the political path. Here's the technical one - and the most important thing about it is something this guide hasn't said plainly enough yet: **the layers are independently adoptable.** A service layer implementation doesn't have to use a unit of work, domain or selector class on day one. You can pick one layer, land it over a release or two, build the team's confidence, and then split things out further. Here's the order that works, refined by years of people making this migration:

1) **Service layer first.** It's the highest immediate value: sweep business logic out of your controllers, batch classes, and triggers into classes ending in `Service`. Remember, technically a service is just a class with some static methods in it - it's the *meaning and responsibility* that matters. Don't worry if you can't follow every convention (even bulkification) on day one; moving the logic behind a service boundary is already the big step. One hard rule though: **don't mark a service class `global` (or its methods `@AuraEnabled`/`@InvocableMethod`) until you're happy with its method signatures** - those are one-way doors you can't walk back through.

2) **Sweep out your inline SOQL.** You don't even need `fflib_SObjectSelector` yet - just start naming classes with the `Selector` suffix and re-home every query into methods on those classes. You lose the consistency benefits of the base class initially, but you get reuse and encapsulation immediately. Extending the base class for *some* selectors and not others is completely fine while you migrate.

3) **Move trigger code into Domain classes** - or do this *first* if your org is heavily trigger-driven. The domain base class has an override for every trigger event, so all your existing trigger logic has an obvious new home, and the [trigger state configuration](/domain-layer/implementing/) replaces any hand-rolled static-boolean recursion guards you find along the way. This is often the best layer for winning over other developers: the code visibly becomes more object-oriented at low risk (assuming decent test coverage), and anyone familiar with trigger-handler or "wrapper" patterns takes to it quickly.

4) **Application factory and mocking last.** Separation of concerns is a *prerequisite* for mocking, so don't introduce the factory before things are factored well enough for it to pay off. Two shortcuts while you get there: mocking *just the service layer* early gives you a quick boost writing tests for controllers and batch classes; and the unit of work's object list doesn't need every object in your org - it only needs the objects the areas you've migrated actually touch. Grow it as you go.

And a mechanical checklist for the factory step: hunt down every place that explicitly writes `new MyDomain(...)`, `new MySelector()` or `new fflib_SObjectUnitOfWork(...)` and replace those with the corresponding `Application` factory calls. You *don't* have to chase down service references the same way - if you followed the service-stub convention from [chapter 8](/service-layer/implementing/), the static service class is already the only thing callers ever referenced, and it delegates through the factory internally.

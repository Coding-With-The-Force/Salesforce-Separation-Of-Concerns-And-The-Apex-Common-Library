...
The answer to this is simple: without Separation of Concerns, there is no unit testing. It just simply isn't possible. To unit test you need to be able to [create stub (mock) classes](https://developer.salesforce.com/docs/atlas.en-us.apexcode.meta/apexcode/apex_testing_stub_api.htm) to send into your class you are testing via [dependency injection](https://en.wikipedia.org/wiki/Dependency_injection) (or through the use of a factory, more on this in the next section). If all of your concerns are in one class (DML transactions, SOQL queries, service methods, domain methods, etc) you cannot fake anything. Let's take a look at a couple examples to illustrate this problem:

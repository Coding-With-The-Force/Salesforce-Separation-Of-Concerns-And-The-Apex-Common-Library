...
2) Start small, take this on a story-by-story basis. Say you have a new story to update some UI in your org, with business owner buy-in, tack on a few extra points to the story to switch it to start using SoC and Unit Testing. Then, MAKE SURE YOUR TESTS ARE INCREDIBLE AND YOU TRUST THEM! If your tests are incredible you never have to ask for permission again... think about it, wtf does the average business person know about code. As long as you don't introduce any new bugs you could change half the codebase and they'd never even know. If your tests are golden, your ability to change on a whim is as well. Make those test classes more trustworthy than your dog (or cat I guess... but I wouldn't trust a cat).
...
And a mechanical checklist for the factory step: hunt down every place that explicitly writes `new MyDomain(...)`, `new MySelector()` or `new fflib_SObjectUnitOfWork(...)` and replace those with the corresponding `Application` factory calls. 

---

## Additional Information

If the above didn't make how to implement mocking via separation of concerns any clearer, [make sure to check out the video covering the subject here.](https://youtu.be/-esf8Q_Vp7U)

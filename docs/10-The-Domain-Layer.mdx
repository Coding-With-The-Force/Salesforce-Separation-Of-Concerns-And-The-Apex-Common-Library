...
<Figure caption={<><b>Only the Selector and the Unit of Work touch the database.</b> The Domain never does - which is exactly why a domain class can be unit tested without a single line of DML or SOQL. Also, while this diagram does not show it, and it's less common, Domain classes can directly call Unit of Work and Selector classes. It does not need to always be delegated through a service class.</>}>
...
Below is a real trigger of the kind that shows up in almost every org. It works. Somehow, it passed a code review. It also
...
<Without files="1" testable="0%" reusable="0%">

<CodeFile name="AccountTrigger.trigger">
...
<With files="4" testable="100%" reusable="100%">

<CodeFile name="AccountsServiceImpl.cls">
...
<CodeFile name="ContactsSelector.cls">
...
The right-hand version is four files instead of one, which feels like more work initially
until you have to start updating your trigger and merge functionality from multiple apps within it. At that point things start to become difficult to manage, testing becomes challenging, and the need to structure in an easier to upkeep manner becomes more apparent.
That's where the Apex Common Library comes in handy. Implement your domain class, never update the trigger itself again, and gain the flexibility of creating distinct methods that you can easily test and call from wherever you need them.   

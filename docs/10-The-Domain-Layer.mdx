...
**_Method Names -_** Method names for database transaction should use the onTransactionName naming convention (Example: `onAfterInsert`). If the method is not related to a database transaction it should be descriptive to indicate what domain logic is being executed within it (Example: `determineCaseStatus`).  

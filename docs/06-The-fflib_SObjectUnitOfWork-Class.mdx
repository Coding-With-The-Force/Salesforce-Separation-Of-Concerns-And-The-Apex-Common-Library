...
If you would like your DML operations to always operate in user mode when using your Unit of Work, you would just declare that in your Application class (which we covered in detail [here](/application/fflib-application/)) and use a custom class extension of the ```fflib_Application.UnitOfWorkFactory``` like the one shown below ```UserModeUnitOfWorkFactory```.    

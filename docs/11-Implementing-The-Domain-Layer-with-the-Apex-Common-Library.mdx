...
Domains don't get one. `fflib_SObjectDomain` is already the abstraction, and
the factory is what makes a domain mockable - you register a stub against the
`SObjectType`, not against a hand-written `ICases`. Services are the layer that
genuinely needs an interface.
...
write them. Don't let the sample code convince you they're required.
:::
...
Before looking at the API, it's worth knowing the problem it was built to kill. In orgs that grew through years of iterative projects, you'll almost always find some version of this hand-rolled in a trigger handler:

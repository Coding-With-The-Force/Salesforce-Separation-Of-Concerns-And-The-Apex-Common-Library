...
## The whole class, line by line

All four factories written out at once. The notes walk down the file; the file
stays where it is.
...
instantiate. You write your *own* class — conventionally called `Application` —
...
The constructor takes a `List<SObjectType>` and that order is load-bearing.
When `commitWork()` runs it inserts in exactly this sequence, which is how a
Contact can be registered before its Account exists and still get the right
`AccountId`.
...
<SourceNote lines="15-22" label="Service" title="An interface pointing at an implementation">

This is the seam. Everywhere in your codebase you ask for
`Task_Service_Interface`, and this map decides what you actually get. In
production it hands back `Task_Service_Impl`. In a test it hands back whatever
stub you registered instead — which is the entire trick behind Apex Mocks, and
why [chapter 17](/apex-mocks/implementing/) keeps insisting you can't skip
this class.

</SourceNote>

<SourceNote lines="24-32" label="Selector" title="SObjectType to the class that queries it">

Same idea, keyed on `SObjectType` rather than an interface. Its job is to
answer "given a Case, who knows how to query Cases?" You'll mostly call this
indirectly — it exists as a separate factory because the domain factory needs
it, which is the next note.

</SourceNote>

<SourceNote lines="34-44" label="Domain" title="Why this one takes another factory">

Look at the first argument: `Application.selector`. The domain factory is the
only one that receives another factory, and the reason is worth pausing on.

When you call `Application.domain.newInstance(recordIds)` you're handing it a
...
[chapter 11](/domain-layer/implementing/) shows you writing one.

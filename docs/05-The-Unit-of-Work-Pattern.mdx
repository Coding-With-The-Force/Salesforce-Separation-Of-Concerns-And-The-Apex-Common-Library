...
"But wait," I hear you say, "Salesforce already rolls back my transaction when something fails. Why do I need a savepoint at all?" Here's the trap: **the platform's automatic rollback only happens when the request fails - and the moment you write a try/catch, you're preventing the request from failing.**

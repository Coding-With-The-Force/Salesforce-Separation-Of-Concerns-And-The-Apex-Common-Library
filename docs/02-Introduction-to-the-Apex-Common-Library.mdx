...
2) **[fflib_SObjectDomain.cls](https://github.com/apex-enterprise-patterns/fflib-apex-common/blob/master/sfdx-source/apex-common/main/classes/fflib_SObjectDomain.cls) -** This houses the base class that all Domain classes you create will extend. The many methods within this class serve to make your life considerably easier when building your domain classes out (you will typically create a domain for each object that requires a trigger). For more information, check out the [Apex Common Domain Layer Implementation Guide](/domain-layer/implementing/) for more details.  
3) **[fflib_SObjectSelector.cls](https://github.com/apex-enterprise-patterns/fflib-apex-common/blob/master/sfdx-source/apex-common/main/classes/fflib_SObjectSelector.cls) -** This houses the base class that all Selector classes you create will extend. The many methods within this class will serve to make things simpler when implementing a selector class for the various objects in your org. For more information, check out the [Apex Common Selector Layer Implementation Guide](/selector-layer/implementing/).  
4) **[fflib_SObjectUnitOfWork.cls](https://github.com/apex-enterprise-patterns/fflib-apex-common/blob/master/sfdx-source/apex-common/main/classes/fflib_SObjectUnitOfWork.cls) -** This houses the logic to implement the [Unit of Work design pattern](https://www.codeproject.com/Articles/581487/Unit-of-Work-Design-Pattern) in your code. There are a ton of useful methods within it that will make your life developing on the platform quite a bit simpler. For more information on the fflib_SObjectUnitOfWork class and the concept itself, please refer to the [guide on how to use the Unit of Work Pattern in Salesforce](/unit-of-work/pattern/).

Honorable mentions beyond the big four: [fflib_QueryFactory](https://github.com/apex-enterprise-patterns/fflib-apex-common/blob/master/sfdx-source/apex-common/main/classes/fflib_QueryFactory.cls) (the fluent query builder underpinning the selector layer, covered in [chapter 14](/selector-layer/implementing/)) and [fflib_SecurityUtils](https://github.com/apex-enterprise-patterns/fflib-apex-common/blob/master/sfdx-source/apex-common/main/classes/fflib_SecurityUtils.cls) (one-call CRUD and field-level security checks, covered in [chapter 6](/unit-of-work/fflib-sobjectunitofwork/)). And the Apex Common Library has a sibling in the same GitHub organization worth knowing about: [force-di](https://github.com/apex-enterprise-patterns/force-di), a dependency injection framework this guide covers in its [final part](/force-di/pattern/).
...
2) [Polymorphism](https://en.wikipedia.org/wiki/Polymorphism_(computer_science)) - When a class uses overloaded methods or overrides an inherited class's methods. 
